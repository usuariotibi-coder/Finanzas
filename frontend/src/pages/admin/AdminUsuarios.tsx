...
export default function AdminUsuarios() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setRecentUsers((prev) => [createdUser, ...prev].slice(0, 10));
      setSuccess(`Usuario creado: ${createdUser.full_name} (${createdUser.email})`);
      resetForm();
    } catch (err) {
      setError(formatRegisterError(err instanceof Error ? err.message : 'No se pudo registrar.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 p-4 shadow-sm sm:p-6">
        ...
        <h1 className="mt-1 text-xl font-semibold text-slate-900">Alta de usuarios</h1>
        <p className="mt-2 text-sm text-slate-600">
          Solo administradores pueden crear cuentas. El rol se asigna automaticamente por departamento.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          <h2 className="text-lg font-semibold text-slate-900">Usuarios creados en sesion</h2>
          <p className="mt-1 text-sm text-slate-600">Ultimos 10 registros realizados desde esta pantalla.</p>
          ...
          {recentUsers.length === 0 ? (
            <div className="mt-6 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
              Aun no has registrado usuarios.
            </div>
          ) : (
              ...
              {recentUsers.map((item) => (
                <li key={item.id} className="rounded-lg border border-slate-200 px-3 py-2">
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    {item.department} · {roleLabels[item.role]}
                  </p>
                </li>
              ))}
            </ul>
